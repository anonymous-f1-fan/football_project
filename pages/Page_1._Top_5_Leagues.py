...
with st.echo(code_location='below'):
    ...
    st.sidebar.header("Анализ топ-5 европейских лиг")

    """В этой части проекта используются данные о разных лигах мира за 2008-2016 года."""

    st.subheader("Раздельный анализ лиг")
    st.sidebar.subheader("Раздельный анализ лиг")

    """Сначала предлагаю ознакомиться со статистикой по каждому чемпионату отдельно, а именно: посмотреть, сколько всего голов забивалось в рассматриваемом чемпионате за несколько сезонов."""
    ...
    st.subheader("Сравнение лиг")
    st.sidebar.subheader("Сравнение лиг")
    ...
    st.subheader("Поиск общих трендов")
    st.sidebar.subheader("Поиск общих трендов")

    """Теперь, когда мы посмотрели, сколько в среднем за матч забивается голов, я предлагаю посмотреть, как распределяются 
    ...
    """Каждая точка на данном графике соответсвует отдельному матчу, то есть количеству забитых домашней и гостевой командой голов.
    Чтобы было лучше видно, сколько матчей завершалось с тем или иным счётом, я добавил немного колебаний, чтобы матчи с каким-то 
    ...
    Тогда, чтобы подтвердить гипотезу, хотелось бы, чтобы после построения регрессии соответсвующая прямая из нуля пошла вверх, в 
    ...
    plt.title('Linear regression on total goals')
    plt.xlabel('Total goals')
    plt.ylabel('Difference')
    st.pyplot(fig)

    """После построения регрессии мы получили именно то, что хотели: в среднем разница мячей положительная. 
