...
with st.echo(code_location='below'):
    ...
    "**4.** Если бот заснул и не отвечает, то просто перейдите по ссылке (https://share.streamlit.io/anonymous-f1-fan/tg_bot/bot.py), чтобы разбудить его."
    st.markdown("**5.** Напишите **/model1**, чтобы выбрать первую модель (менее точную, где используются все полевые игроки), или "
                "**/model2**, чтобы выбрать вторую модель (где рассматриваются только центральные нападающие).")
    st.markdown("**6.** Напишите фамилию игрока, для которого хотите посмотреть характеристики, его общий рейтинг в FIFA и предсказываемый рейтинг. "
    ...
    st.markdown("**7.** Далее Вы можете посмотреть информацию про другого игрока или выбрать другую модель, написав **/model** "
